...
import { FileHeart, Lock, Printer, ShieldAlert } from "lucide-react";
...
export const Route = createFileRoute("/app/clinic/prescriptions")({
  ...
  head: () => ({
    meta: [
      { title: "Prescriptions — MediUnivers Clinic" },
      { name: "description", content: "Every prescription issued, ready to view or print." },
    ],
  }),
...
interface ConsultationApiDto {
  ...
  patient: { id: number; name: string; patientNumber: string; phone: string | null };
...
const PHARMACY_STATUS_STYLE: Record<string, string> = {
...
function PrescriptionsPage() {
  ...
  function loadList() {
    ...
    apiFetch<ConsultationApiDto[]>("/api/clinic/consultations/prescriptions")
      .then(setList)
      .catch((err) =>
        setLoadError(err instanceof ApiError ? err.message : "Couldn't load prescriptions."),
      );
  }
  ...
  useEffect(() => {
    ...
    Promise.all([
      apiFetch<ConsultationApiDto>(`/api/clinic/consultations/${id}`),
      ...
      <div className="space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
          <Button asChild variant="outline">
            <Link to="/app/clinic/prescriptions">Back to prescriptions</Link>
          </Button>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              {detail.patient.phone ? (
                <p className="text-xs text-muted-foreground">{detail.patient.phone}</p>
              ) : null}
            </div>
            <div>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Every prescription issued from a completed consultation — dispensing itself happens on the
          Pharmacy module's dispensing screen.
        </p>
      </div>
      ...
      {loadError ? (
        ...
        <Card className="p-10 text-center text-sm text-muted-foreground">
          <FileHeart className="mx-auto mb-2 h-6 w-6" /> No prescriptions issued yet.
        </Card>
      ) : (
            ...
            <div key={c.id} className="flex flex-wrap items-center gap-3 p-4">
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium">
                  {rxCode(c)} · {c.patient.name}
                </p>
                <p className="truncate text-xs text-muted-foreground">
                  {c.patient.patientNumber} · Dr. {c.doctor.fullName} · {c.startedAt.slice(0, 10)} ·{" "}
                  {c.prescriptionItems.length} item(s)
                </p>
              </div>
              <Badge variant="outline" className={PHARMACY_STATUS_STYLE[c.pharmacyStatus] ?? ""}>
                {PHARMACY_STATUS_LABELS[c.pharmacyStatus] ?? c.pharmacyStatus}
              </Badge>
              <Button asChild size="sm" variant="outline">
                <Link to="/app/clinic/prescriptions" search={{ id: c.id }}>
                  View / Print
                </Link>
              </Button>
            </div>
